...
<body>
    ...
    <script>
        function changeImageInContactSellerModal(src) {
            const fileExtension = src.split('?')[0].split('.').pop().toLowerCase();
            ...
            if (imageExtensions.includes(fileExtension)) {
                mediaElement =
                    `<img src="${src}" alt="Product Image" class="w-full h-full object-cover rounded-sm shadow-md mb-4">`;
            } else if (videoExtensions.includes(fileExtension)) {
                mediaElement = `
            <video class="w-full h-full object-cover rounded-sm shadow-md mb-4" autoplay muted loop playsinline>
                <source src="${src}" type="video/${fileExtension}">
                Votre navigateur ne supporte pas la lecture de cette vidéo.
            </video>`;
            } else {
                mediaElement =
                    `<img src="products_media/images.jpg" alt="Product Image" class="w-full h-full object-cover rounded-sm shadow-md mb-4">`;
            }

            const mainImageElement = document.getElementById('mainImageInContactSellerModal');
            if (mainImageElement) {
                mainImageElement.innerHTML = mediaElement;
            } else {
                console.error('Element with ID "mainImageInContactSellerModal" not found.');
            }
        }

        function contactSellerModal(event, product) {
            event.preventDefault();
            const host = window.location.origin; // Définir la variable host
