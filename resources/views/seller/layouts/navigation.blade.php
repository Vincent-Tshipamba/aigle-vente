 @php
     $latestConversations = Auth::user()->conversations()->with('messages')->latest()->get();
 @endphp
             ...
             <a class="flex items-center gap-4" href="#" @click.prevent="dropdownOpen = ! dropdownOpen">
                 <span class="hidden text-right lg:block">
                     <span class="block text-sm font-medium text-black dark:text-white">
                         {{ Auth::user()->seller->first_name . ' ' . Auth::user()->seller->last_name }}</span>
                     <span class="block text-xs font-medium">vendeur</span>
                 </span>

                 <span class="h-12 w-12 rounded-full">
                     <img src="{{ asset(Auth::user()->seller->picture ?? 'img/profil.jpeg') }}" alt="User"
                         class="rounded-ful" />
                 </span>
 ...
 <aside
     ...
     <div class="mini mt-20 flex flex-col space-y-2 w-full h-[calc(100vh)]">
         ...
         <!-- Vendeurs -->
         <a href="{{ route('chats') }}"
             class="relative inline-flex hover:ml-4 justify-end pr-5   hover:text-[#e38407] dark:hover:text-[#e38407]  p-3 rounded-full transform ease-in-out duration-300  {{ request()->routeIs('odcusers.index') ? ' text-[#e38407]' : 'dark:text-white text-gray-700' }}">
             <svg class="" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24"
                 height="24" fill="currentColor" viewBox="0 0 24 24">
                 <path fill-rule="evenodd"
                     d="M4 3a1 1 0 0 0-1 1v8a1 1 0 0 0 1 1h1v2a1 1 0 0 0 1.707.707L9.414 13H15a1 1 0 0 0 1-1V4a1 1 0 0 0-1-1H4Z"
