...
@section('content')
    ...
    <form action="{{ route('seller.shops.products.update', ['shop' => $shop->id, 'product' => $product->id]) }}"
        ...
        <div class="grid grid-cols-1 gap-9 sm:grid-cols-2">
                            ...
                            <select id="state" name="state" required
                                ...
                                @foreach ($state as $state)
                                    <option value="{{ $state->id }}"
                                        {{ old('state', $state->id) == $state->id ? 'selected' : '' }}>
                                        {{ $state->name }}
                                    </option>
                                @endforeach
                            </select>
            ...
            <button type="submit"
                class="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:focus:ring-yellow-900">Modifier
                le produit</button>
